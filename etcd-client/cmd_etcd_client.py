...
def main():
    ...
    # Get command
    get_parser = subparsers.add_parser('get', help='Get the value associated with a key')
    get_parser.add_argument('--prefix', default=False, action='store_true', help='Get all the KV pairs starting with a prefix')
    ...
    set_parser.add_argument('--swap', default=False, action='store_true', help='Change the value of an existing key.')
    set_parser.add_argument('--old_value', default=False, action="store", help='The previous value of the key')
    set_parser.set_defaults(which='set')

    # Mkdir command
    mkdir_parser = subparsers.add_parser('mkdir', help='Create a new directory')
    mkdir_parser.add_argument('key', action="store", help='The directory you want to add')
    mkdir_parser.set_defaults(which='mkdir')

    # Ls command
    ls_parser = subparsers.add_parser('ls', help='List a directory')
    ls_parser.add_argument('key', action="store", help='The directory you want to list')
    ls_parser.set_defaults(which='ls')

    # Rm command
    rm_parser = subparsers.add_parser('rm', help='Remove a key or a directory')
    rm_parser.add_argument('key', action="store", help='The key or directory you want to remove')
